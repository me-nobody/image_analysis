...
mmid = getImageID();
//roiManager("Reset");
//Analyse the voronoi signal to find the image regions
//run("Analyze Particles...", "size=1000-Infinity pixel add");
...
for(i=0;i<rcount;i++){
	...
	selectImage(imid);
	roiManager("Select",i+rcount);
	run("Measure");
	
	}


tcount = roiManager("Count");
selectImage(imid);
...
for(i=rcount;i<tcount;i++){
	roiManager("Select",i);
	getSelectionBounds(x, y, width, height);
	if(x==0 || y==0 || width+x >getWidth()-1||height+y>getHeight()-1){
		continue;
		}
	run("Measure");
	...
	print(title,"\tnum of cells\t",name,"\tave intensity per pixel CH3\t",CH3,"\ttotal intensity CH3\t",CH3raw,"\tnum of foci CH3\t",CH3foci);
